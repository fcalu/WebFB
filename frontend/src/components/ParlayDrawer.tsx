...
export default function ParlayDrawer({
  ...
  useEffect(() => {
    if (!open) return;
    fetch(`${API_BASE}/leagues`).then(r => r.json()).then((d: ApiLeagues) => setLeagues(d.leagues ?? [])).catch(() => setLeagues([]));
  }, [open, API_BASE]);
  ...
  function removeLeg(i: number) {
    setLegs(prev => prev.filter((_, idx) => idx !== i).concat([{ league: "", teams: [], home: "", away: "", odds: {} }]).slice(0,4));
  }
  ...
  async function generate() {
    ...
    try {
      const payload = {
        legs: legs.filter(L => L.league && L.home && L.away).slice(0, 4).map(L => ({
          league: L.league, home_team: L.home, away_team: L.away, odds: L.odds
        }))
      };
      if (payload.legs.length < 2) {
        setErr("Agrega al menos 2 partidos."); setLoading(false); return;
      }
      const r = await fetch(`${API_BASE}/parlay/suggest`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  ...
  return (
    <>
      ...
      <div
        onClick={onClose}
        style={{
          position: "fixed", inset: 0, background: open ? "rgba(0,0,0,.45)" : "transparent",
          pointerEvents: open ? "auto" : "none", transition: "background .25s", zIndex: 70
        }}
      />
      {/* drawer */}
      <div
        style={{
          position: "fixed", top: 0, right: 0, height: "100%", width: "min(560px,95vw)",
          background: "#0b1020", borderLeft: "1px solid rgba(255,255,255,.12)",
          transform: open ? "translateX(0)" : "translateX(110%)", transition: "transform .25s",
          zIndex: 80, padding: 16, overflowY: "auto"
        }}
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
          <div style={{ fontWeight: 900, fontSize: 18 }}>⚡ Generador de Parley {isPremium ? "Premium" : ""}</div>
        ...
        {!isPremium && (
          <div style={{ ...panel, marginBottom: 10, borderColor: "rgba(234,179,8,.4)" }}>
            ...
            <div style={{ marginTop: 8 }}><a href="#" style={{ color: "#a5b4fc", textDecoration: "underline" }}>Más info</a></div>
          </div>
        )}
          ...
          return (
            <div key={i} style={{ ...panel, marginBottom: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                <button onClick={() => removeLeg(i)} style={{ ...pill, cursor: "pointer" }}>Eliminar</button>
              </div>
              <div style={{ display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr 1fr" }}>
                ...
                <div>
                  ...
                  <input list={`home_${i}`} value={L.home} onChange={e => patch(i, { home: e.target.value })} style={input} placeholder="Escribe para buscar…" />
                ...
                <div>
                  ...
                  <input list={`away_${i}`} value={L.away} onChange={e => patch(i, { away: e.target.value })} style={input} placeholder="Escribe para buscar…" />
                ...
                {(["1","X","2","O2_5","BTTS_YES"] as (keyof Odds)[]).map((k, idx) => (
                  <div key={idx}>
                    <div style={{ color: "#a5b4fc", fontSize: 12, fontWeight: 800, marginBottom: 6 }}>
                      {k==="1"?"1 (Local)":k==="X"?"X (Empate)":k==="2"?"2 (Visitante)":k==="O2_5"?"Más de 2.5": "BTTS Sí"}
                    </div>
                    <input
                      type="text"
                      inputMode="decimal"
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
          <button onClick={generate} disabled={loading} style={{ ...pill, cursor: "pointer", borderColor: "#7c3aed", background: "linear-gradient(135deg,#7c3aed55,#5b21b655)" }}>
            {loading ? "Generando…" : "⚙️ Generar con IA"}
          </button>
          <button onClick={() => setOut(null)} style={{ ...pill, cursor: "pointer" }}>Limpiar resultado</button>
        </div>

        {/* salida */}
        {err && <div style={{ marginTop: 10, color: "#fecaca", border: "1px solid rgba(239,68,68,.35)", padding: 10, borderRadius: 12 }}>{err}</div>}

        {out && (
          <div style={{ ...panel, marginTop: 12 }}>
            <div style={{ fontWeight: 900, marginBottom: 10 }}>Resultado del Parley</div>
            {out.legs.map((L, idx) => (
              <div key={idx} style={{ padding: 10, borderRadius: 10, background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", marginBottom: 8 }}>
                ...
                <div style={{ marginTop: 4 }}>
                  Pick: <b>
                    {L.pick.market === "1X2"
                      ? (L.pick.selection==="1"?"Gana local":L.pick.selection==="2"?"Gana visitante":"Empate")
                      : L.pick.market === "Over 2.5"
                      ? "Más de 2.5 goles"
                      : L.pick.market === "BTTS" ? "Ambos anotan (Sí)" : `${L.pick.market} — ${L.pick.selection}`}
                  </b>
                  {" · Prob: "}<b>{pct(L.pick.prob_pct)}</b>
                  {" · Cuota usada: "}<b>{L.used_odd ?? "—"}</b>
                  {L.ev != null && <>{" · EV: "}<b>{fmt2(L.ev)}</b></>}
                </div>
              </div>
            ))}
            <div style={{ marginTop: 10, display: "grid", gap: 10, gridTemplateColumns: "repeat(2,1fr)" }}>
              <div style={pill}>Prob. combinada: <b style={{ marginLeft: 6 }}>{pct(out.combined_prob_pct)}</b></div>
              <div style={pill}>Cuota justa: <b style={{ marginLeft: 6 }}>{fmt2(out.combined_fair_odds)}</b></div>
              <div style={pill}>Cuota parley (ingresada): <b style={{ marginLeft: 6 }}>{out.combined_used_odds ?? "—"}</b></div>
              <div style={pill}>EV parley: <b style={{ marginLeft: 6 }}>{out.combined_ev != null ? fmt2(out.combined_ev) : "—"}</b></div>
            </div>
            <div style={{ marginTop: 10, opacity: .85 }}>{out.summary}</div>
          </div>
        )}
      </div>
    </>
