...
function ExtraSuggestions({ data }: { data: PredictResponse }) {
  ...
  const pUnder35 = clamp01(1 - pO25 * 0.75);
  const pFavU35 = lowScoring ? clamp01(pFav * pUnder35) : 0;

  const suggestions = [
    { key: "1X", title: "Doble oportunidad: 1X", prob: p1x, reason: "Cobras con Local o Empate (más segura que 1 limpio)." },
    { key: "X2", title: "Doble oportunidad: X2", prob: px2, reason: "Cobras con Empate o Visitante (útil si el local no es muy fuerte)." },
    { key: "12", title: "Doble oportunidad: 12", prob: p12, reason: "Gana cualquiera (evitas el empate)." },
    { key: "1X+BTTS", title: "1X & BTTS Sí", prob: p1x_btts, reason: "Partido equilibrado con goles de ambos." },
    { key: "X2+O25", title: "X2 & Over 2.5", prob: px2_o25, reason: "Visitante no pierde y partido con ritmo ofensivo." },
    ...(lowScoring ? [{ key: "Fav+U35", title: `Favorito (${fav}) & Under 3.5`, prob: pFavU35, reason: "Favorito por poco y tendencia a pocos goles." }] : []),
  ]
    .filter((s) => s.prob > 0)
            ...
            >
              ...
              {(() => {
                const { ev, edge, usedOdd } = calcEdgeAndEV(
                  data.best_pick,
                  data.probs,
                  ({} as Odds) // si quieres que use tus cuotas de arriba: pon "odds"
                );
