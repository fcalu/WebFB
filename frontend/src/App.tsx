...
const BG_LIGHT = "linear-gradient(180deg, #ffffff 0%, #f6f7fb 100%)";
...
// Tarjeta gradiente (varía ligeramente en claro)
const cardGradientStyle = (dark: boolean): React.CSSProperties => ({
  borderRadius: 20,
  padding: 20,
  background: dark
    ? "linear-gradient(135deg, rgba(168,85,247,.18), rgba(99,102,241,.18))"
    : "linear-gradient(135deg, rgba(99,102,241,.14), rgba(59,130,246,.10))",
  border: dark ? "1px solid rgba(99,102,241,.28)" : "1px solid rgba(59,130,246,.25)",
...
function Calculadora({
  ...
  // cuotas opcionales (para mantener tu sección de cuotas responsiva)
    ...
    try {
      const res = await fetch(`${API_BASE}/predict`, {
        ...
        body: JSON.stringify({
          ...
          home_team: home,
          away_team: away,
          // puedes incluir odds si tu backend las acepta:
          odds,
        }),
      });
  ...
  return (
    <>
      ...
      {/* ====== Cuotas opcionales (responsive y balanceadas) ====== */}
      <div style={{ ...panelStyle(dark), marginBottom: 18 }}>
        ...
        <div style={{ marginTop: 10, display: "flex", gap: 10, flexWrap: "wrap" }}>
          ...
          <div style={{ fontSize: 12, opacity: 0.8, alignSelf: "center" }}>
            Sugerencia: ingresa las cuotas ~5 horas antes del partido para mejor
            precisión.
          </div>
        </div>
      </div>
