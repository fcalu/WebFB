...
try:
   ...
   def top_matches_payload(date: Optional[str] = None):
    ...
    for event_container in data.get("events", []):
        ...
        league_slug = event_container.get("season", {}).get("slug", "")
        league_name = event.get("league", {}).get("name", "Desconocida")
        
        # Filtramos solo ligas que podríamos tener en la base de datos (simplificación)
        if "league" not in league_slug: continue
        ...
        if odds_list:
            ...
            # Nota: Obtener O2_5 y BTTS es más complejo y no está estandarizado en este endpoint
            # Por simplicidad, solo incluimos 1X2 por ahora, pero la estructura ya está ahí.
            # Los nombres de los equipos y la liga son lo más importante para tu App.
